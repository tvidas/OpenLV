...
public class OperatingSystem
{
    ...
    {
        String regData = LiveViewLauncher.queryRegistry("HKLM\\NEWSOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", "ProductName", "REG_SZ");

        if(regData == null)  //key could not be found -- probably a win9x image, so check that path for prod name
        {
            regData = LiveViewLauncher.queryRegistry("HKLM\\NEWSOFTWARE\\Microsoft\\Windows\\CurrentVersion", "ProductName", "REG_SZ");
        }
        return regData;
    }
