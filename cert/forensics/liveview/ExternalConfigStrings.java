...
public class ExternalConfigStrings 
{
	...
	//private final Properties myprops = getPropertiesFromHomeDir("LiveView.properties");
	...
	{
		try{
			myprops = getPropertiesFromHomeDir("LiveView.properties");
		}catch(Exception ex){
			if(!haveWarned){
				System.err.println("No liveview.ini found at " + System.getProperty("user.home") + " using defaults\n");
				LogWriter.log("No liveview.ini found at " + System.getProperty("user.home") + " using defaults\n" );
			}
		}
	}
	...
	private Properties getPropertiesFromHomeDir(String propFileName) throws IOException {
		...
		//props.load(new FileInputStream("c:/Temp/LiveView.ini"));
