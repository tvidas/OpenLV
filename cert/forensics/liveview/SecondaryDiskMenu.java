...
public class SecondaryDiskMenu extends JDialog
{
	...
	public SecondaryDiskMenu(JFrame p, File wd)//LiveViewLauncher p)//String displayText, String underlyingValue)
	{
		parent = p;
		workingDir = wd;
	}
		...
		ActionListener sourceModeListener = new ActionListener()
		{
			...
			{   
				...
				if(e.getActionCommand().compareTo("PhysicalDisk") == 0)
				{
					...
					PhysicalDiskInfo[] physicalDeviceItems = LiveViewLauncher.getPhysicalDeviceItems();
					...
					for(int i = 0; i < physicalDeviceItems.length; i++)
					{
						...
						displayString = currPDI.getModel() + " (" + Math.ceil(currPDI.getSize()/(LiveViewLauncher.BYTES_PER_GIG)) + " GB)";
